...
router.post('/', authenticate, authorize('car_owner', 'admin'), async (req, res) => {
  try {
...
router.put('/:id', authenticate, authorize('car_owner', 'admin'), async (req, res) => {
  try {
    const car = await CarViewModel.update(req.params.id, req.userId, req.body);
    res.json({ success: true, data: car });
  } catch (error) {
    res.status(error.status || 500).json({ success: false, message: error.message });
  }
...
router.delete('/:id', authenticate, authorize('car_owner', 'admin'), async (req, res) => {
  try {
